...
**Ros2Drun** — автономная миссия для квадрокоптера на ROS 2 + PX4 Offboard. Дрон взлетает, проходит маршрут по лабиринту с использованием ROS 2-навигации и ArUco/локальной системы координат, снимает кадры с бортовой камеры и после посадки ищет целевые объекты через YOLOv8.

Проект создан в рамках **MISIS Robotics Week** на хакатоне **«Автономные дроны с ИИ» от ООО «Сверх»**. Команда **МИСИСхАлабуга** заняла **1 место**: решение объединило автономный полет, работу с ArUco/локализацией, съемку кадров и последующую обработку снимков на борту.

### Контекст мероприятия

| Параметр | Значение |
|----------|----------|
| Мероприятие | MISIS Robotics Week, хакатон по автономным дронам с ИИ от Сверх |
| Тип | Хакатон |
| Дата | 18 февраля 2026 года, в рамках MISIS Robotics Week 18-22 февраля 2026 года |
| Сайт мероприятия | https://misis.ru/events/6739/ |
| Количество участников | 50 |
| Призовой фонд | 35 000 рублей |

### Результат команды

| Параметр | Значение |
|----------|----------|
| Команда | МИСИСхАлабуга |
| Место | 1 место |
| Состав | Морев Семен, Табаков Максим, Буянов Петр |
| Результат участия | набор инструментов, сувенирная продукция от спонсоров, стажировка в ООО «Сверх» |
| Цифровой артефакт | этот репозиторий |
...
`body_drun.py` запускает финальный хакатонный сценарий: взлет с `auto_arm=True`, полет по «змейке» через относительные смещения в `body`, фоновую съемку и обработку снимков после посадки. Такой режим использовался как устойчивый маршрутный слой поверх общей ROS 2/Offboard-инфраструктуры проекта.
...
По умолчанию в `main.py` выбран сценарий `ArucoFramesDroneController` — полет по точкам в системе координат `aruco_map`. Чтобы переключить режим, закомментируйте текущую стратегию и раскомментируйте нужную:
...
- Навигация по точкам в системе координат `aruco_map` через ArUco-ориентиры.
- Переключаемые стратегии миссии: `aruco_map`, список точек, относительные смещения в `body`.
- Стабильный маршрутный слой для прохождения трассы «змейкой».
...
![alt text](/docs/demo/telegram-cloud-photo-size-2-5262604800975639245-y.jpg)

## 6. Архитектура и структура проекта

### 6.1. Подход к навигации
...
В репозитории сохранены оба направления: `aruco_frames_flight.py` для работы с `aruco_map` и `body_drun.py` для стабильного прохождения трассы по относительным смещениям. Финальный маршрутный скрипт был написан за два дня до защиты. Изначально он не прошел трассу, но на защите мы поправили автоарминг при взлете, после чего дрон уверенно пролетел маршрут и сделал снимки. YOLOv8n запускалась уже во время приземления/после полета, чтобы не нагружать процессор в критический момент управления дроном.
